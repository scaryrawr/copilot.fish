...
complete -c copilot -l connect -x -d 'Connect directly to a remote session (optionally specify session ID or task ID)'
...
complete -c copilot -l resume -x -d 'Resume from a previous session (optionally specify session ID or task ID)'
...
complete -c copilot -l secret-env-vars -r -d 'Environment variable names whose values are stripped from shell and MCP server environments and redacted from output'
